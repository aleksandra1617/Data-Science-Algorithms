###################################################################################################################
#    ID3 CLASSIFICATION                                                                                           #
#    NOTE! I have written the algorithm with a small and easy to use dataset (Play Tennis dataset)                #
...
#                                                                                                                 #
# 2) DATA SETS                                                                                                    #
#    TEST RUN DATA SETs: This data can be found by navigating to the TEST RUN - DATA SETS region in this file.    #
#    It is a part of the ID3 file because I believe it would be useful to have an example of how the ID3 code     #
#    works with a very small data set and also provides an opportunity to better understand what the code         #
#     is doing. To have a look at ID3 applied to a small data set just make a call the test_run()                 #
...
#                                                                                                                 #
# 4) USES                                                                                                         #
#    A) Classify labeled data, example approving loans and credit cards, etc.                                     #
...
Pacman_DATASET_BY_ATTRIB_DICT = {"Walls":  ["1010", "0010", "0010", "1010"],
                                "Food":    ["0101", "1010", "0101", "0101"],
                                "Ghost 1": ["00000000", "00100000", "00000000", "00000000"],
                                "Ghost 2": ["00000000", "00000001", "00000001", "00000001"],
                                "Visible": ["0", "1", "0", "0"]}
...
Pacman_TARGET_ATTRIB_LIST = ["1", "0", "4", "4"]
...
TRAIN_DATA_SIZE = len(Pacman_TARGET_ATTRIB_LIST)
# endregion
...
class ID3DecisionTree:
    def __init__(self):
        ...
        # In this way, no tree traversal is needed to find the next available node.
    ...
    def get_next_branch_occurrences(self, dataset_by_attrib_dict, target_list):
        ...
        for attrib_name in self.linked_attributes:
            del subdict[attrib_name]
    ...
    def calc_entropy(self, attrib_uv_count, overall):
        ...
        for key in attrib_uv_count.keys():
            ...
            if attrib_uv_count[key] != 0:
                fraction = attrib_uv_count[key] / float(overall)
                target_attrib_calc = fraction * math.log(fraction, 2)  # EDIT!!
    ...
    def build_node(self, name, completed_branches):
        if name == "None" or completed_branches is None:
            return
        ...
        # if all outcome branches for the attribute are completed, then the attribute is complete and its outcomes
        ...
        # Create all the outcome nodes.
        for outcome_name in self.dataset_occurrence_dict[name]:
            # Here a template outcome node is created from which each of the outcome nodes will be copied
            # to solve the issue where node = Node() does not create a new node but instead re-instantiates
            # the same node every time so non of the data fro the previously made node is retained.
            temp_outcome = Node(outcome_name)
            new_outcome_node = copy.deepcopy(temp_outcome)
            ...
            derived_nodes.append(new_outcome_node)
            ...
            if outcome_name not in completed_outcomes:
                ...
                self.active_branch_nodes.append(new_outcome_node)

            # del new_outcome_node removes the node from memory even if it has been given to a list the node is
            # passed by reference so new_outcome_node and new_outcome_node in the list share the same memory address
            # therefore we can't delete one without deleting the other.
            #del new_outcome_node.derived_nodes
            #new_outcome_node.derived_nodes = []  # EDIT!!
            #del new_outcome_node
    ...
    def build_tree_chunk(self, dataset_by_attrib_dict, target_attrib_list):
        ...
        target_uv_data = list(set(target_attrib_list))  # TODO: FIX POSSIBLE EFFICIENCY DECREASE
        ...
        self.linked_attributes.append(next_node_data["name"])
    ...
    def build_tree(self, dataset_by_attrib_dict, target_attrib_list):
        ...
        while len(self.active_branch_nodes) != 0:
            ...
            sub_attrib_dict, sub_tar_list = self.get_next_branch_occurrences(dataset_by_attrib_dict, target_attrib_list)    # ISSUE IS HERE OR AFTER THIS
...
def test_run_algorithm():
    print " " * 10, " << ID3 CLASSIFICATION ALGORITHM TEST RUN >> ", " " * 10

    tree = ID3DecisionTree()
    tree.build_tree(Pacman_DATASET_BY_ATTRIB_DICT, Pacman_TARGET_ATTRIB_LIST)

    # APPLY CLASSIFICATION
    # The index of the entry in the dataset.
    entry_index = 0
    tree.classify(entry_index, Pacman_DATASET_BY_ATTRIB_DICT)
